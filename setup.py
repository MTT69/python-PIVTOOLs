...
class BuildCLibraries(build):
    ...
    def build_c_libraries(self):
        ...
        # No FFTW: the codelet FFT engine (libbulkxcorr2d) replaced it and the
        # GSL/FFTW k-space + marquadt fitters were removed, so nothing links FFTW.
        ...
        # No FFTW to strip any more (the codelet engine replaced it); the base
        # flags are the platform compile/link flags as-is.
        base_compile = list(self.extra_compile)
        bulk_link = list(self.extra_link)
        ...
        # --- Build libfusedwarp (no external deps — only OpenMP + math) ---
        # SIMD flags for the warp kernel ONLY.
        ...
        if use_msvc:
            ...
            cmd_fw = [
                ...
                *warp_simd_flags,
                ...
                # bulk_link carries -lm -fopenmp and, on macOS, the libomp
                # -rpath + -isysroot the runtime needs to find libomp.dylib
                # (Homebrew LLVM is not on the default search path). On Linux it
                # reduces to ["-lm", "-fopenmp"].
                *bulk_link,
            ]
        if warp_simd_flags:
