#!/usr/bin/env python3
"""
Generate synthetic ChArUco stereo calibration images with known ground-truth
camera and inter-camera parameters for stereo calibration recovery tests.

The dotboard renderer that lived here was removed on 2026-08-28: it rounded
every dot centre to an integer pixel and drew aliased discs, so it limited
the detector it was meant to test (0.29 px quantisation, a 1.8 mm stereo
baseline bias). Dotboard accuracy is checked on a real board with a known
setup instead.
...
def main():
    ...
    np.savez(
        ...
        R_stereo=R_stereo,
        T_stereo=T_stereo,
        rvecs_cam1=np.array([p[0] for p in poses_cam1_char]),
        tvecs_cam1=np.array([p[1] for p in poses_cam1_char]),
        rvecs_cam2=np.array([p[0] for p in poses_cam2_char]),
        tvecs_cam2=np.array([p[1] for p in poses_cam2_char]),
        image_width=W,
        image_height=H,
    )
