...
def _fft(bld: Builder, xs: list, inverse: bool) -> list:
    ...
    for s in range(m):
        T = [
            bld.cmul_const(subs[j][s], cmath.exp(sign * 2j * math.pi * (j * s) / n))
            for j in range(r)
        ]
        for p in range(r):
            acc = None
            for j in range(r):
                term = bld.cmul_const(
                    T[j], cmath.exp(sign * 2j * math.pi * (j * p) / r)
                )
...
def validate(sizes):
    ...
    for n in sizes:
        ...
        if tag != "OK":
            ok = False
        print(
            f"N={n:>4}  cfft={e1:.2e}  icfft={e2:.2e}  rfft={e3:.2e}  irfft={e4:.2e}  {tag}"
        )
    return ok
...
class Isa:
    ...
    f_suffix: bool = True  # append the 'f' float literal suffix (False -> double)
    ...
    def lit(self, c: float) -> str:
        ...
        if "e" not in s and "." not in s:
            s += ".0"
        if self.f_suffix:
            # a bare 'f' on a double codelet would silently round every twiddle
            # to float precision, defeating the point of the double ISA
            s += "f"
        return f"set1({s})" if self.simd_ops else s

    def add(self, a, b):
        return f"vadd({a}, {b})" if self.simd_ops else f"({a} + {b})"

    def sub(self, a, b):
        return f"vsub({a}, {b})" if self.simd_ops else f"({a} - {b})"
    ...
    def mulc(self, a, c):
        return (
            f"vmul({a}, {self.lit(c)})" if self.simd_ops else f"({a} * {self.lit(c)})"
        )
...
ISAS = {
    ...
    # Double-precision scalar renderer, used by the k-space LM ensemble fitter
    # (libkspacefit). That fitter's output is Sigma, gated against a float64
    # oracle at |dSigma| < 1e-8 — a float32 transform cannot reach it. Same IR,
    # same validated mixed-radix decomposition, rendered to `double` with
    # unsuffixed literals. Nothing in the correlation path uses it.
    "scalar_d": Isa(
        "scalar_d", "double", 1, "scalar_d", simd_ops=False, f_suffix=False
    ),
...
ALL_TRANSFORMS = ("rfft", "cfft", "icfft", "irfft")


def render_header(sizes, isas, transforms=ALL_TRANSFORMS) -> str:
    out = [
        "// AUTO-GENERATED by gen_codelet.py -- do not edit.",
    ...
    if intrin:
        ...
        out += [
            "// --- GCC/Clang vector extensions (lower to AVX2/SSE on x86 / NEON on arm64) ---"
        ]
        for iname in vecext:
            isa = ISAS[iname]
            out.append(
                f"typedef float {isa.ctype} __attribute__((vector_size({isa.lanes * 4})));"
            )
        out.append("")
    for n in sizes:
        half = n // 2
        for iname in isas:
            ...
            if "rfft" in transforms:
                ...
                out.append(
                    _emit(isa, sig, ops, outs, {f"xr{i}": f"xr[{i}]" for i in range(n)})
                )
            ...
            # irfft (c2r)
            if "irfft" in transforms:
                ops, _, outs = build_irfft(n)
                sig = (
                    f"static inline void irfft{n}_{s}"
                    f"(const {t}* Xr, const {t}* Xi, {t}* y)"
                )
                binds = {
                    **{f"Xr{k}": f"Xr[{k}]" for k in range(half + 1)},
...
def main():
    ...
    ap.add_argument(
        "--transforms",
        nargs="+",
        default=list(ALL_TRANSFORMS),
        choices=list(ALL_TRANSFORMS),
        help="Transforms to emit. The k-space LM fitter needs only cfft, and "
        "restricting to it keeps the unrolled double header (and its compile "
        "time) to a quarter of the full set.",
    )
    args = ap.parse_args()
    ...
    if len(intrin) > 1:
        raise SystemExit(
            "emit at most one intrinsic SIMD ISA per header (macros collide)"
        )
    hdr = render_header(args.sizes, args.isa, args.transforms)
    with open(args.emit, "w") as f:
        f.write(hdr)
    print(
        f"wrote {args.emit}  ({len(hdr.splitlines())} lines, "
        f"sizes={args.sizes}, isa={args.isa}, transforms={args.transforms})"
    )
