    ...
    double min_k, double max_k)
{
    ...
    // Scan positive k starting ONE bin above DC. The k=0 bin of a
    // mean-subtracted correlation spectrum is ~0, so testing it against
    // an F_max-relative threshold trips immediately and collapses k_max
    // to min_k for every window — fatal on small planes where the min_k
    // ring holds <10 points (and silently starves the fit on large ones).
    double k_max = max_k;
    ...
    // Floor: also catches the no-signal case where even the first
    // post-DC bin is sub-threshold (k_max would be k_axis[center] = 0).
