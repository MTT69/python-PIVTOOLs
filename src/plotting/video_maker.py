...
class PlotSettings:
    ...
    upscale: tuple | None = None  # e.g. (H_out, W_out) or None (keep native)
...
def _compute_global_limits_from_files(files, pick, settings: PlotSettings):
    ...
    for f in files:
        ...
        # arrs shape (R,3,H,W) - take first run
        if arrs.ndim == 4:
            arr = arrs[0, 0 if pick == "ux" else 1]
            b_mask = arrs[0, 2]
        else:
            # fallback
            arr = arrs[0]
...
class FFmpegVideoWriter:
    ...
    def write(self, rgb_frame_uint8):
        self.proc.stdin.write(rgb_frame_uint8.tobytes())
...
def make_velocity_video_from_mats(
    folder: str | Path,
    pick: str = "uy",  # 'ux' or 'uy'
...
if __name__ == "__main__":
    ...
    meta = make_velocity_video_from_mats(
        folder=frames_dir,
        pick="ux",  # 'ux' or 'uy'
