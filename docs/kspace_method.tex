...
\begin{enumerate}[leftmargin=2em]
    ...
    %
    \begin{equation}
        w(\kv) = \max\!\Bigl(\exp\!\bigl(-\lvert\kv\rvert^2 / 0.15^2\bigr),\; 0.1\Bigr)
...
Two complementary estimates of $k_\mathrm{max}$ are computed per axis.

\textbf{Profile threshold.}
The noise-corrected reference spectrum $\Fclean$ is sampled along each wavenumber axis (at $k_y = 0$ and $k_x = 0$).
The profile-based $k_\mathrm{max}$ is the wavenumber at which $\lvert\Fclean\rvert$ first drops below $1\%$ of its DC value:
%
\begin{equation}
...
A model-based bound follows from setting $\lvert T(k_\mathrm{max})\rvert = 1/\mathrm{SNR}$ in equation~\eqref{eq:tmodel}:
%
\begin{equation}
    k_\mathrm{max}^\mathrm{var} = \sqrt{\frac{\ln(\mathrm{SNR})}{2\pi^2\, \hat\Sigma}},
    \label{eq:kmax_var}
\end{equation}
%
where $\hat\Sigma$ is the initial variance estimate from the 1D regression~\eqref{eq:1d_regression}.
Beyond this wavenumber the transfer function has decayed into the noise.

\textbf{Combined bound.}
The per-axis $k_\mathrm{max}$ is the most conservative of the two estimates, capped below Nyquist:
%
\begin{equation}
    k_{\mathrm{max},x} = \min\!\bigl(k_{\mathrm{max},x}^\mathrm{prof},\;
    k_{\mathrm{max},x}^\mathrm{var},\; k_\mathrm{cap}\bigr),
    \label{eq:kmax_combined}
\end{equation}
%
where $k_\mathrm{cap} = 0.45$~cycles/pixel when soft weighting is used and $k_\mathrm{cap} = 0.30$ otherwise.
...
\begin{enumerate}[leftmargin=2em, itemsep=2pt]
    ...
    \item Determine the per-axis fitting boundary $k_{\mathrm{max},x}$, $k_{\mathrm{max},y}$ from the profile decay of $\Fclean$, the variance-based bound~\eqref{eq:kmax_var}, and the Nyquist cap~\eqref{eq:kmax_combined}.
