...
\begin{enumerate}[leftmargin=2em]
    ...
    \item \textbf{Weighting:} A Gaussian taper
    %
    \begin{equation}
        w(\kv) = \max\!\Bigl(\exp\!\bigl(-\lvert\kv\rvert^2 / (2 \cdot 0.15^2)\bigr),\; 0.1\Bigr)
        \label{eq:stage1_weight}
    \end{equation}
    %
    concentrates fitting effort where signal-to-noise is highest.
    The scale $0.15$~cycles/pixel reflects the signal-to-noise transition for typical $\sigma \approx 2$~px particles, where the Gaussian signal has decayed to approximately $e^{-2}$ of its DC value.
    The floor of $0.1$ ensures that the solver retains sensitivity at high wavenumbers where only the $N_0 \cdot \Pnoise$ term remains --- this is what constrains $N_0$.

    \item \textbf{Solver:} Bounded nonlinear least-squares (trust-region reflective) with bounds $A \in [0.01, 10]$, $\sigma_x, \sigma_y \in [0.3, 20]$~px, and $N_0 \in [0, 1]$ (all relative to DC).
\end{enumerate}
...
The signal-to-noise ratio of $\Fclean$ is estimated as
%
\begin{equation}
    \mathrm{SNR} = \frac{\lvert\Fclean(\mathbf{0})\rvert^2}
    {\operatorname{median}\!\bigl(\lvert\Fclean(\kv)\rvert^2 \;\big\vert\; 0.4 < \lvert\kv\rvert < 0.5\bigr)}.
    \label{eq:snr}
\end{equation}
%
The annular ring at $0.4 < \lvert\kv\rvert < 0.5$ is rotationally symmetric (avoiding DFT corner artifacts) and samples wavenumbers where the signal Gaussian has negligible amplitude.
Windows with $\mathrm{SNR}$ below a threshold are rejected as having insufficient signal.

\subsection{Adaptive Fitting Region}

The fitting extends to the wavenumber where signal and noise are comparable:
%
\begin{equation}
    k_\mathrm{max} = \sqrt{\frac{\ln(\mathrm{SNR})}{2\pi^2\, \hat\Sigma}},
    \label{eq:kmax}
\end{equation}
%
derived from setting $\lvert T(k_\mathrm{max})\rvert = 1/\mathrm{SNR}$ in the model~\eqref{eq:tmodel}.
Beyond this wavenumber, the transfer function is dominated by noise amplification and contributes no useful information.
The fitting region is elliptical:
%
\begin{equation}
...
\end{equation}
%
with per-axis $k_\mathrm{max}$ values computed from the respective $\hat\Sigma_{xx}$ and $\hat\Sigma_{yy}$.
...
%
\begin{equation}
    w_\mathrm{snr}(\kv) = \frac{\lvert\Fclean(\kv)\rvert}{\sqrt{P_\mathrm{noise\, floor}}}.
    \label{eq:w_snr}
\end{equation}
%
...
The solver minimises $\sum_\kv \lvert r(\kv)\rvert^2$ over the five parameters $(\mu_x, \mu_y, \Sigma_{xx}, \Sigma_{yy}, \Sigma_{xy})$ using bounded trust-region reflective least-squares on the stacked real and imaginary parts.
...
\begin{enumerate}[leftmargin=2em, itemsep=2pt]
    ...
    \item Form the transfer function: $T(\kv) = F_{AB}(\kv)\, /\, \Fclean(\kv)$.

    \item Normalise: $T_\mathrm{norm}(\kv) = T(\kv)\, /\, T(\mathbf{0})$.

    \item \textbf{Stage~2:} Fit five parameters $(\mu_x, \mu_y, \Sigma_{xx}, \Sigma_{yy}, \Sigma_{xy})$ from weighted nonlinear least-squares on the complex $T_\mathrm{norm}$.
