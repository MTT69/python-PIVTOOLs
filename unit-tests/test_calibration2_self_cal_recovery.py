"""End-to-end stereo self-calibration recovery for calibration2.

Builds a synthetic stereo pair, renders particle images on a laser sheet with a
KNOWN ``(z_offset, tilt_x, tilt_y)``, then drives the calibration2 ``self_cal.run``
...
``CameraModel`` and the recovered params land in the calibration2 world frame.
...
def _stereo_models():
    """A symmetric calibration2 CameraModel pair looking at the origin."""
    w, h = IMAGE_SIZE
