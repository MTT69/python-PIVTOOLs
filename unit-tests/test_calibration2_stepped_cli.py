"""S8 — calibration2 stepped CLI commands, end-to-end through the headless surface.

Drives the REAL ``detect_stepped_mono2_command`` / ``detect_stepped_stereo2_command``
...
import pivtools_cli.calibration2_cli as cli
from pivtools_gui.calibration2.record import load_mono, load_stereo
...
from test_calibration2_stepped_mono import (  # noqa: E402
    PEAK_COLS,
    POSE_RVECS,
    SPACING_MM,
    STEP_MM,
...
def test_mono_pinhole_cli_writes_reloadable_model(mono_scene, tmp_path, monkeypatch):
    ...
    path = cli.detect_stepped_mono2_command(
...
def test_mono_cli_requires_spec(mono_scene, tmp_path, monkeypatch):
    ...
    with pytest.raises(SystemExit, match="stepped-spec"):
        cli.detect_stepped_mono2_command(_mono_args(tmp_path, model_type="pinhole"))
...
def test_stereo_pinhole_cli_writes_reloadable_model(stereo_scene, tmp_path, monkeypatch):
    ...
    path = cli.detect_stepped_stereo2_command(args)
    assert path.exists()
