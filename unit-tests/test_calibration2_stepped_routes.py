"""S4 — calibration2 stepped Flask routes (the GUI API), end-to-end.
...
import pivtools_gui.calibration2.app.stepped_views as sv
...
from test_calibration2_stepped_stereo import (  # noqa: E402
    PEAK_COLS,
    POSE_RVECS,
    RVEC_EXTRA,
    SPACING_MM,
...
def client(scene, tmp_path, monkeypatch):
    ...
    app = Flask(__name__)
    app.register_blueprint(sv.calibration2_stepped_bp)
...
def _detect(client, cameras):
    r = client.post("/calibration2/stepped/detect_sequence", json={
    ...
    done = _wait(client, f"/calibration2/stepped/detect_sequence/status/{body['job_id']}")
...
def test_pose_detection_strips_internal_keys(client, scene):
    ...
    r = client.get("/calibration2/stepped/sequence_pose_detection",
...
def test_identify_pose_level_and_snap(client, scene):
    ...
    r = client.post("/calibration2/stepped/identify_pose_level", json={
    ...
    r = client.post("/calibration2/stepped/snap_fiducial", json={
...
def test_mono_generate_end_to_end(client, scene, tmp_path):
    sid, _ = _detect(client, [1])
    r = client.post("/calibration2/stepped/generate_model", json={
        "sequence_id": sid, "stereo": False,
        "cameras": {"1": _spec(scene, 1)}})
    body = r.get_json()
    assert r.status_code == 200, body
    done = _wait(client, f"/calibration2/stepped/generate_model/status/{body['job_id']}")
    assert done["status"] == "completed", done
