#!/usr/bin/env python3
"""
Generate synthetic PIV image pairs for a 2-camera Couette flow test.

Velocity field:
  ux(y_phys) = u_max * y_norm   m/s  (antisymmetric, crosses zero at centre)
...
Output:
  unit-tests/couette_multicam/
    Cam1/B00001_A.tif, B00001_B.tif
    Cam2/B00001_A.tif, B00001_B.tif
    ground_truth.npz

Usage:
    python unit-tests/generate_couette_multicam.py
"""
...
OUTPUT_DIR = Path(__file__).resolve().parent / "couette_multicam"
...
RVEC_CAM1 = np.array([0.025, -0.015, 0.01], dtype=np.float64)
RVEC_CAM2 = np.array([-0.015, 0.025, -0.01], dtype=np.float64)
...
def _generate_camera_images(camera_matrix, dist_coeffs, rvec, tvec,
    ...
    for pair_idx in range(1, NUM_PAIRS + 1):
        ...
        # Couette velocity: ux = U_MAX * y_norm (antisymmetric)
        y_norm = world_pts[:, 1] / (H_phys / 2.0)
        ux_phys = U_MAX * y_norm
...
def main():
    ...
    print(f"Couette u_max={U_MAX} m/s, uy_const={UY_CONST} m/s, dt={DT} s")
