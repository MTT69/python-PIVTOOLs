"""Ensemble Reynolds-stress calibration (calibration2).
...
def test_isotropic_reduces_to_scalar():
    ...
    m = build_scale_factor_record(
        camera=1, origin_px=(0, 0), px_per_mm=p, image_size=IMAGE_SIZE, dt=dt,
        x_dir="right", y_dir="down").camera_model
    coords = np.stack(np.meshgrid([0.0, 10.0], [0.0, 20.0]), axis=-1)
    UU = np.full((2, 2), 4.0); VV = np.full((2, 2), 9.0); UV = np.full((2, 2), 2.0)
...
def test_mirror_flips_cross_term_only():
    ...
    coords = np.stack(np.meshgrid([0.0, 10.0], [0.0, 20.0]), axis=-1)
    UU = np.full((2, 2), 4.0); VV = np.full((2, 2), 9.0); UV = np.full((2, 2), 2.0)
    mr = build_scale_factor_record(camera=1, origin_px=(0, 0), px_per_mm=p,
                                   image_size=IMAGE_SIZE, dt=dt, x_dir="right", y_dir="down").camera_model
    ml = build_scale_factor_record(camera=1, origin_px=(0, 0), px_per_mm=p,
                                   image_size=IMAGE_SIZE, dt=dt, x_dir="left", y_dir="down").camera_model
...
def test_anisotropic_tensor_transform_exact():
    ...
    M = np.array([[0.02 * np.cos(theta), -0.03 * np.sin(theta)],
                  [0.02 * np.sin(theta), 0.03 * np.cos(theta)]])  # anisotropic + rotated
    model = _AffineModel(M=M, c=np.array([5.0, -2.0]))
    coords = np.array([[100.0, 200.0], [300.0, 50.0]])
    UU = np.array([4.0, 1.0]); VV = np.array([9.0, 16.0]); UV = np.array([2.0, -3.0])
...
def test_runio_ensemble_roundtrip(tmp_path):
    ...
    p, dt = 10.0, 5e-4
    m = build_scale_factor_record(camera=1, origin_px=(0, 0), px_per_mm=p,
                                  image_size=IMAGE_SIZE, dt=dt, x_dir="right", y_dir="down").camera_model
    H = W = 2
    xg = np.array([[0.0, 10.0], [0.0, 10.0]]); yg = np.array([[0.0, 0.0], [20.0, 20.0]])
    er = np.empty((1,), dtype=[("ux", "O"), ("uy", "O"),
                               ("UU_stress", "O"), ("VV_stress", "O"), ("UV_stress", "O")])
    er["ux"][0] = np.full((H, W), 8.0); er["uy"][0] = np.full((H, W), 4.0)
...
def test_ensemble_shape_mismatch_raises(tmp_path):
    ...
    import pytest
    dt = 5e-4
    m = build_scale_factor_record(camera=1, origin_px=(0, 0), px_per_mm=10.0,
                                  image_size=IMAGE_SIZE, dt=dt, x_dir="right", y_dir="down").camera_model
    # coords are 2x2; the ensemble velocity field is 3x3 -> mismatch.
    xg = np.array([[0.0, 10.0], [0.0, 10.0]]); yg = np.array([[0.0, 0.0], [20.0, 20.0]])
    er = np.empty((1,), dtype=[("ux", "O"), ("uy", "O")])
    er["ux"][0] = np.full((3, 3), 8.0); er["uy"][0] = np.full((3, 3), 4.0)
    scipy.io.savemat(str(tmp_path / "ensemble_result.mat"), {"ensemble_result": er})
...
def test_resolve_dt_precedence_and_no_silent_default():
    ...
    rec_a = build_scale_factor_record(camera=1, origin_px=(0, 0), px_per_mm=10.0,
                                      image_size=IMAGE_SIZE, dt=0.5).board_meta["dt"]
    rec_b = build_scale_factor_record(camera=2, origin_px=(0, 0), px_per_mm=10.0,
                                      image_size=IMAGE_SIZE, dt=2.0).board_meta["dt"]
