...
def _views(true_board, n=14, seed=0):
    ...
    for v in range(n):
        ...
        tvec = np.array(
            [rng.uniform(-20, 20), rng.uniform(-20, 20), 600 + rng.uniform(-40, 40)]
        )
...
def test_release_beats_flat_on_held_out_view():
    ...
    assert (
        rms_r < rms_f
    ), f"released held-out {rms_r:.4f} not better than flat {rms_f:.4f}"
