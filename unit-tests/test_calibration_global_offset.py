"""Global-frame offset baked into the model (calibration).
...
def test_offset_adds_to_coordinates_not_velocity():
    ...
    u, v = APPLY.calibrate_displacements(
        m, pts, np.array([[3.0, 0.0], [0.0, 4.0]]), dt=1.0
    )
    assert np.allclose(u, [3.0e-3, 0.0]) and np.allclose(
        v, [0.0, -4.0e-3]
    )  # +Y up -> -4
...
def test_two_camera_stitch_via_baked_offset():
    ...
    pairs = [
        {
            "camera_a": 1,
            "camera_b": 2,
            "pixel_on_a": [200.0, 0.0],
            "pixel_on_b": [300.0, 0.0],
        }
    ]
    ...
    g1 = APPLY.calibrate_coordinates(
        r1.camera_model,
        np.array([[200.0, 0.0]]),
        offset_mm=r1.world_frame.world_offset_mm,
    )[0]
    g2 = APPLY.calibrate_coordinates(
        r2.camera_model,
        np.array([[300.0, 0.0]]),
        offset_mm=r2.world_frame.world_offset_mm,
    )[0]
