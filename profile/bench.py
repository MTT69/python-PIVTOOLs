...
def cmd_scaling(args: argparse.Namespace) -> int:
    ...
    if args.plots_only:
        bench_plots.plot_scaling(
            Path(args.plots_only), Path(args.plots_only).resolve().parent
        )
        return 0
    ...
    configs = bs.build_config_list(
        ...
        total_cores,
        max_workers,
        worker_sweep_threads=args.worker_threads,
    ...
    if args.dry_run:
        bs.print_dry_run(
            configs, args.iterations, args.n_images, batch_hint=args.batch_hint
        )
        return 0
    ...
    csv_path = (
        Path(args.resume) if args.resume else bc.timestamped_path("scaling", "csv")
    )
    bs.run_sweep(
        args.config,
        args.dataset,
        ...
        image_format=_split_csv_arg(args.image_format),
        start_index=args.start_index,
        worker_memory=args.worker_memory,
        fftw_wisdom=args.fftw_wisdom,
...
def cmd_profile(args: argparse.Namespace) -> int:
    ...
    print(
        f"  per-pair: read {b['read']:.2f} + filter {b['filter']:.2f} "
    ...
    if ks and ks.get("xcorr_fft_ms") is not None:
        print(
            f"  kernel split: FFT {ks['xcorr_fft_ms']:.3f} ms, "
...
def cmd_compare(args: argparse.Namespace) -> int:
    ...
    if pa is not None and pb is not None:
        ...
        if guard["ok"]:
            ...
            print(
                "  -> comparison proceeds but the figures may not be apples-to-apples."
            )
    else:
    ...
    if a["profile_json"] and b["profile_json"]:
        ...
        p = bench_plots.plot_budget_ab(
            payload_a,
            payload_b,
            out_dir / f"{day}-ab-fftw-vs-codelet-budget.png",
        )
        wrote.append(p)
...
def _add_common(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        "--config", required=True, help="Base config.yaml (PIV settings pinned)"
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument(
        "--sweep",
        choices=["threads", "workers", "matrix", "oversub", "all"],
        default="all",
    )
    s.add_argument(
        "--total-cores",
    ...
    s.add_argument(
        "--worker-memory", default=None, help="Per-worker memory limit, e.g. '3.2GB'"
    )
    ...
    s.add_argument(
        "--resume", metavar="CSV", default=None, help="Append to / resume a CSV"
    )
    s.add_argument(
        "--plots-only",
        metavar="CSV",
    ...
    _add_common(pr)
    pr.add_argument(
        "--mode", choices=["instantaneous", "ensemble"], default="instantaneous"
    )
    pr.add_argument(
        "--threads",
        type=int,
    ...
    pr.add_argument(
        "--out",
        default=None,
        help="JSON output path (default: results/profile_<ts>.json)",
    )
