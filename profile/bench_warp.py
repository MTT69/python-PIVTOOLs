...
from test_fused_warp import (  # noqa: E402
    make_grid_image, make_poiseuille_predictor, make_window_centres,
)
...
def _load_lib() -> ctypes.CDLL:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"libfusedwarp not found at {path} — run `python setup.py build`")
    lib = ctypes.CDLL(str(path))
    lib.fused_symmetric_warp_batch.restype = ctypes.c_int
    lib.fused_symmetric_warp_batch.argtypes = [
        _NDP, _NDP, _NDP, _NDP,            # imgs_a, imgs_b, outs_a, outs_b
        _NDP, _NDP,                        # pred_dy, pred_dx
        ctypes.c_int, ctypes.c_int, ctypes.c_int,   # N, H, W
        ctypes.c_int, ctypes.c_int,        # nPY, nPX
        _NDP, _NDP,                        # ctrs_y, ctrs_x
        ctypes.c_int, ctypes.c_int,        # interp_mode, shared_predictor
    ]
...
def _make_batch(N: int, H: int, W: int):
    ...
    imgs_a = np.ascontiguousarray(
        np.stack([make_grid_image(H, W, spacing=16) for _ in range(N)]), dtype=np.float32)
    imgs_b = np.ascontiguousarray(
        np.stack([make_grid_image(H, W, spacing=20) for _ in range(N)]), dtype=np.float32)
    pred_dy = np.ascontiguousarray(np.stack([pdy] * N), dtype=np.float32)
    pred_dx = np.ascontiguousarray(np.stack([pdx] * N), dtype=np.float32)
    return imgs_a, imgs_b, pred_dy, pred_dx, \
        np.ascontiguousarray(ctrs_y, np.float32), np.ascontiguousarray(ctrs_x, np.float32), nPY, nPX
...
def _run(lib, data, N, H, W, interp_mode, impl):
    ...
    ret = lib.fused_symmetric_warp_batch(
        imgs_a, imgs_b, out_a, out_b, pred_dy, pred_dx,
        N, H, W, nPY, nPX, ctrs_y, ctrs_x, interp_mode, 0)
    if ret != 0:
...
def main(argv=None):
    ...
    os.environ["OMP_NUM_THREADS"] = str(args.threads)  # must precede the first kernel call
    ...
    print(f"CPU: {cpu}   batch N={N}  {H}×{W}  threads={args.threads}  iters={args.iters}\n")
    print(f"{'mode':<9}{'impl0 ref ms':>13}{'impl1 opt ms':>13}{'speedup':>9}"
    ...
    for mode in modes:
        ...
        print(f"{_MODE_NAME[mode]:<9}{ms_ref:>13.2f}{ms_opt:>13.2f}"
