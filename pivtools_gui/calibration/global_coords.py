"""calibration.global_coords — N-camera global coordinate stitching (chain math).
...
pixel->physical step uses calibration's own model back-projection
(``apply.calibrate_coordinates``) — so there is NO v1 model format, and NO ``-y``
flip (the sign is carried by the fitted model, per the calibration contract).
