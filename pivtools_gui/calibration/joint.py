...
class _JointBA:
    ...
    def __init__(self, nominal: np.ndarray, observations: list, cams: Sequence[int],
                 view_keys: Sequence[ViewKey], free_rows: Sequence[int], mode: str):
    ...
    def jac(self, x):
        ...
        for k in self.view_keys:
            ...
            for n_i in range(M):
                ...
                if self._per_board == 3:
                    for cc in range(3):
                        data.append(dP[n_i, :, cc]); ridx.append([base, base + 1])
                        cidx.append([c + cc, c + cc])
                else:
                    data.append(dP[n_i, :, 2]); ridx.append([base, base + 1]); cidx.append([c, c])
            r0 += 2 * M
...
def _seed_reproj(cam: int, views: Sequence[int], det_of: Dict[ViewKey, DetectionResult],
                 global_of: Dict[ViewKey, np.ndarray], row_of: Dict[Tuple[int, int], int],
                 nominal: np.ndarray, K: np.ndarray, dist: np.ndarray) -> float:
    ...
    for v in views:
        ...
        try:
            ...
            return np.inf
        proj, _ = cv2.projectPoints(obj, cv2.Rodrigues(R)[0], np.asarray(t).reshape(3), K, dist)
        sq += float(((proj.reshape(-1, 2) - img) ** 2).sum())
        n += len(rows)
    return float(np.sqrt(sq / n)) if n else np.inf


def _bootstrap_camera(cam: int, views: Sequence[int],
                      det_of: Dict[ViewKey, DetectionResult],
                      global_of: Dict[ViewKey, np.ndarray], row_of: Dict[Tuple[int, int], int],
                      nominal: np.ndarray, image_size: Tuple[int, int],
                      distortion_model: DistortionModel, fix_aspect_ratio: bool, fix_k3: bool):
    ...
    Ka, da, *_ = fit_intrinsics(objs, imgs, image_size, distortion_model,
                                fix_aspect_ratio, fix_k3, use_release_object=False)
    candidates.append(("calibrateCamera(union=%d dots)" % sum(len(o) for o in objs), Ka, da))
    ...
    if len(common) >= _MIN_COMMON_CORE:
        ...
        Kb, db, *_ = fit_intrinsics([objp] * len(views), imgp, image_size, distortion_model,
                                    fix_aspect_ratio, fix_k3, use_release_object=True)
        candidates.append(("calibrateCameraRO(common-core=%d)" % len(common), Kb, db))

    scored = [(lab, K, dist, _seed_reproj(cam, views, det_of, global_of, row_of, nominal, K, dist))
              for (lab, K, dist) in candidates]
    scored.sort(key=lambda s: s[3])
    lab, K, dist, r = scored[0]
    if not np.isfinite(r):
        raise ValueError(f"joint: camera {cam} — no intrinsic seed produced a finite reprojection")
    return K, dist, lab
...
def run_joint(
    ...
    if board_release not in ("full3d", "z_only", "none"):
        raise ValueError(f"board_release must be full3d|z_only|none, got {board_release!r}")
    if distortion_model != DistortionModel.STANDARD or not fix_aspect_ratio or not fix_k3:
    ...
    keys = sorted({tuple(int(c) for c in g)
                   for gi in global_index.values() for g in gi})
    row_of = {g: i for i, g in enumerate(keys)}
    N = len(keys)
    nominal = np.array([[g[0] * spacing_mm + ox, g[1] * spacing_mm + oy, 0.0] for g in keys])

    det_of = {k: detections_by_cam[k[0]][k[1]] for k in global_index}
    global_of = {k: np.asarray(v, dtype=np.int64).reshape(-1, 2) for k, v in global_index.items()}
    ...
    if (datum_camera, datum_view) not in view_keys:
        raise ValueError(f"joint: datum ({datum_camera},{datum_view}) not in the resolved grid")
    ...
    image_size_by_cam = {c: (int(image_size_by_cam[c][0]), int(image_size_by_cam[c][1]))
                         for c in cams}
    ...
    for c in cams:
        ...
        K, dist, how = _bootstrap_camera(
            c, views_c, det_of, global_of, row_of, nominal, image_size_by_cam[c],
            distortion_model, fix_aspect_ratio, fix_k3)
    ...
    for k in view_keys:
        ...
        R, t = fit_pose(board[rows], det_of[k].image_points, K_by_cam[k[0]], dist_by_cam[k[0]],
                        planar=True)
        pose_by_view[k] = (R, t)
    ...
    rows_of_view = {k: [row_of[tuple(int(c) for c in g)] for g in global_of[k]] for k in view_keys}

    def _overall_rms(Kc, dc, poses, brd) -> float:
        sq, n = 0.0, 0
        for k in view_keys:
            R, t = poses[k]
            proj, _ = cv2.projectPoints(brd[rows_of_view[k]], cv2.Rodrigues(R)[0],
                                        np.asarray(t).reshape(3), Kc[k[0]], dc[k[0]])
        ...
        for it in range(max_iters):
            ...
            for r in released_rows:
                ...
                for (cam, view, u, v) in obs_by_row[r]:
                    ...
                    o, d = _pixel_rays_world(np.array([[u, v]]), K_by_cam[cam],
                                             dist_by_cam[cam], R, t)
                    origins.append(o)
                    dirs.append(d[0])
            ...
            for k in view_keys:
                R, t = fit_pose(board[rows_of_view[k]], det_of[k].image_points,
                                K_by_cam[k[0]], dist_by_cam[k[0]], planar=False)
                pose_by_view[k] = (R, t)
    ...
    info = {"bootstrap": boot_info, "rms_after_alternation": rms_alt,
            "n_released": int(len(released_rows)), "n_union": N,
    ...
    if run_bundle and board_release != "none" and len(released_rows) >= 3:
        ...
        sol = least_squares(ba.residuals, x0, jac=ba.jac, method="trf", x_scale="jac",
                            tr_solver="lsmr", ftol=1e-12, xtol=1e-12, gtol=1e-12,
        ...
        if rms_bundle <= rms_pre + 1e-9:
            ...
            info["bundle"] = {"status": int(sol.status), "nfev": int(sol.nfev),
                              "rms": rms_bundle, "applied": True}
        else:
            log.warning("joint: final bundle did not improve RMS (%.4f -> %.4f) — keeping "
                        "the alternation result", rms_pre, rms_bundle)
            info["bundle"] = {"status": int(sol.status), "nfev": int(sol.nfev),
                              "rms": rms_bundle, "applied": False}

    # ---- assemble per-camera models (datum-view pose) + per-camera RMS ----
    models, per_cam_rms = {}, {}
    for c in cams:
        if (c, datum_view) not in pose_by_view:
            raise ValueError(f"joint: camera {c} did not observe the datum view {datum_view}")
        ...
        for k in view_keys:
            ...
            proj, _ = cv2.projectPoints(board[rows_of_view[k]], cv2.Rodrigues(Rk)[0],
                                        np.asarray(tk).reshape(3), K_by_cam[c], dist_by_cam[c])
            d = proj.reshape(-1, 2) - det_of[k].image_points
            sq += float((d ** 2).sum())
            n += len(rows_of_view[k])
        per_cam_rms[c] = float(np.sqrt(sq / n))
        models[c] = CameraModel(
            K=K_by_cam[c], dist=dist_by_cam[c], R=R, t=np.asarray(t).reshape(3, 1),
            image_size=image_size_by_cam[c], distortion_model=distortion_model, rms=per_cam_rms[c])
    ...
    return JointResult(
        cameras=cams, models=models, board=board_dict, view_poses=pose_by_view,
        world_frame=wf, spacing_mm=float(spacing_mm), board_release=board_release,
        rms_px=float(rms_final), per_camera_rms=per_cam_rms,
        cross_camera_board_agreement_mm=0.0, converged=converged, info=info)
...
def run_joint_polynomial(
    ...
    for cam in cameras:
        ...
        if cam not in image_size_by_cam:
            raise ValueError(f"run_joint_polynomial: image_size_by_cam missing camera {cam}")
        ...
        world = np.column_stack([gi[:, 0] * spacing_mm + ox, gi[:, 1] * spacing_mm + oy,
                                 np.zeros(len(gi))])
