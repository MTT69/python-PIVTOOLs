"""calibration2.self_cal — stereo self-calibration (Wieneke 2005) for calibration2.

Bridges the calibration2 stereo model to the self-contained algorithm core in
...
so its world frame IS the calibration2 clicked origin/+X/+Y frame. The algorithm's
...
calibration2 stays independent of the v1 calibration service layer.
"""
...
def pinhole_from_model(model: CameraModel) -> PinholeCamera:
    """Adapt a calibration2 ``CameraModel`` to the algorithm core's ``PinholeCamera``.

    A pure field copy (same ``K, dist, R, t`` world->camera and ``image_size``), so
    the resulting camera's world frame is the calibration2 clicked frame.
    """
...
def _camera_world_bounds(model: CameraModel) -> Tuple[float, float, float, float]:
    ...
    calibration2 ray-plane intersection. Mirrors the legacy
    ``camera_model_utils.compute_camera_world_bounds`` but off the calibration2 model.
    """
...
def load_particle_pairs(
    ...
    core/cli loaders directly so calibration2 does not depend on the v1 service.
    """
...
def run(
    ...
    *,
    ...
    """Run self-calibration for a calibration2 stereo record.
