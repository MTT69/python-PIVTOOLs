...
## Synthetic Image & Calibration Board Generation

**Canonical tool:** All synthetic PIV particle images and calibration boards are generated by the external EUROPIV SIG tool at `C:\Users\mtt1e23\Downloads\EUROPIV_synthetic_image_generator\SIG\src\sig\`. SIG has its own `CLAUDE.md` in that folder — **all usage instructions, CDL configs, invocation patterns, Cygwin commands, Scheimpflug math, and internals live there**. Read SIG's CLAUDE.md before generating any synthetic data; do not duplicate its content here.

SIG generates:
- Planar 2C and stereo 3C particle image pairs (physically realistic Gaussian PSFs, sensor noise, shared camera model)
- Dot calibration boards
- Cross calibration boards (renders correctly but pivtools' `grid_detection.py` is dots-only, so cross is visual-sanity only)
- Stepped calibration targets (two Z levels, stereo configs)
- End-to-end recovery validation against pivtools' `MultiViewCalibrator` via SIG's `validate_calibration.py`

**SIG cannot generate ChArUco boards** — this is an architectural limit, not a bug. SIG renders only Gaussian splat clusters from `.data` particle files; it cannot produce sharp binary ArUco marker patterns. A Python/OpenCV ChArUco generator is retained as a manual reference tool at `python-PIVTOOLs/manual_tools/calibration_boards_open_cv/generate_synthetic_charuco.py`, alongside an OpenCV dotboard generator kept for the same reason. These two scripts are reference-only; **do not use them for standard verification**, and do not add new synthetic image generation code to this repo — generate via SIG instead.

No in-repo synthetic PIV particle image generation has ever existed. All particle images come from SIG.
...
| Calibration (stepped stereo) | `calibration/app/stepped_board_views.py` | `useSteppedBoardCalibration` | `SteppedBoardCalibration` |
| Calibration (stepped planar) | `calibration/app/stepped_planar_views.py` | `useSteppedPlanarCalibration` | `SteppedPlanarCalibration` |
...
| Image dewarp overlay | `manual_tools/image_dewarp_overlay.py` | N/A (script) | N/A |
...
- **Raw image cache:** `raw_image_cache` stores 4-tuples `(b64_A, b64_B, stats, timestamp)` — no numpy arrays stored (saves ~640MB). Stats are always `{vmin_pct: 0, vmax_pct: 100}` since `numpy_to_base64` handles sqrt + percentile optimisation internally. `jsonify()` runs outside `cache_lock` to avoid serialization contention.
...
numpy_to_base64(arr: np.ndarray, format="png", compress_level=1, jpeg_quality=85, vmin=None, vmax=None) -> str
    # Sqrt variance-stabilising transform + 0.5th-99.5th percentile clip for non-uint8.
    # Dramatically improves contrast for low-count PIV images (12/16-bit cameras).
    # uint8 images (masks, overlays) pass through unchanged.
```
...
| `stepped_board_bp` | `/calibrate/stepped_board` | Stepped board stereo calibration (2-camera) |
| `stepped_planar_bp` | `/calibration/stepped_planar` | Stepped board single-camera 3D calibration (per camera) |
...
- `calibration_planar/planar_calibration_production.py` - Dotboard detection + model. Grid detection delegated to `grid_detection.py`, I/O to `calibration_io.py`. Supports `model_type="pinhole"` (OpenCV) or `"polynomial"` (3rd-order bivariate) via `MultiViewCalibrator(model_type=...)`. **Polynomial mode is datum-only**: the polynomial fit uses only the datum frame regardless of how many frames were detected. If additional frames were detected they are ignored and a warning is returned in `result["warnings"]` (surfaced to the frontend via the job manager) — use pinhole for multi-image calibration. Detection itself still runs on all frames (for overlay UX).
- `calibration_charuco/charuco_calibration_production.py` - ChArUco detection. Board creation delegated to `calibration_io.create_charuco_detector()`, I/O to shared utilities. Supports `model_type="pinhole"` or `"polynomial"` via `ChArUcoCalibrator(model_type=...)`. ChArUco world points are in meters (multiplied by 1000 for polynomial mm input). **Polynomial mode is datum-only**: reads `global_coordinates_datum_frame` from config (fallback `calibration.charuco.datum_frame`, default 1) and fits only on that frame. Multi-frame detection emits a warning in `result["warnings"]`.
- `calibration_poly/polynomial_calibration_production.py` - Polynomial calibration consumer (`PolynomialVectorCalibrator`) + polynomial model fitting from detected points (`fit_polynomial_from_points()`, `save_polynomial_to_config()`) + DaVis XML import helpers (`read_calibration_xml`, `convert_davis_coeffs_to_array`). The fitted model is a generic 3rd-order bivariate polynomial with 10 terms [1, s, s², s³, t, t², t³, s*t, s²*t, s*t²] solved via `np.linalg.lstsq` — not tied to any specific vendor. mm_per_pixel estimated via median nearest-neighbor ratios (cKDTree). **Coordinate convention:** `PolynomialVectorCalibrator` converts uncalibrated coords (1-based, y-up) → raw pixels (0-based, y-down) via `_uncal_to_raw()` before polynomial evaluation, matching the space the polynomial was fitted in. `image_height` stored from config for the conversion. The DaVis XML path (`read_calibration_xml`, `convert_davis_coeffs_to_array`) remains as legitimate LaVision interop for users with existing DaVis Calibration.xml files.
...
- `camera_model_utils.py` - Shared utilities: `load_pinhole_camera(base_dir, cam_num, method)` loads per-camera planar .mat model, `compute_camera_world_bounds(...)` projects image edges to Z=0 world plane. Used by `manual_tools/image_dewarp_overlay.py` and `self_calibration_service.py`.
- `manual_tools/image_dewarp_overlay.py` and `manual_tools/stereo_dewarp_diagnostic.py` - Standalone diagnostic scripts, moved out of the production `calibration/` module (2026-04-16). `image_dewarp_overlay` dewarps raw images from multiple cameras into physical coords and overlays them with interactive coordinate readout + measurement tools. `stereo_dewarp_diagnostic` loads a stereo model .mat, dewarps both cameras to a common world Z-plane, and displays a red-cyan overlay with an interactive Z slider. Both import from `pivtools_gui.calibration.*` and `pivtools_gui.stereo_reconstruction.self_calibration`.
- `services/self_calibration_service.py` - Service layer for stereo self-calibration. Orchestrates camera loading, source image loading (evenly sampled frame A), red-cyan dewarp overlay generation (percentile-based normalization for particle image visibility), and `run_self_calibration()` invocation. **Storage:** Results saved to `{base_path}/calibration/stereo_cam{A}_cam{B}/self_calibration.yaml` (file alongside stereo model, not config.yaml — prevents stale values persisting across datasets). Manual Z-offset entry via `save_manual` route. File cleared when new stereo model is generated. Used by both GUI routes (`self_calibration_views.py`) and CLI (`pivtools-cli self-calibrate`).
- `stereo_reconstruction/stereo_dotboard_calibration_production.py` - Stereo dotboard. Grid detection delegated to `grid_detection.py` (flat-field pipeline, no external detector needed). **Y-frame convention (2026-04-17):** `make_object_points_dynamic` (line 215) and `_match_object_points` (line 271) negate `obj_points[:, 1]` before `cv2.solvePnP`/`stereoCalibrate` so the stored `(R, t)` is physics-up. BFS grid detector locks +Y to image-down by default; the negation restores lab-frame convention at calibration construction time, matching what stepped already does via fiducial clicks and what planar does at output time.
- `stereo_reconstruction/stereo_calibration_base.py` - Stereo base class. Parallel camera image reads via ThreadPoolExecutor. Container format + image finding delegated to `calibration_io.py`. **No camera_placement.png** — only the stepped calibrator generates the interactive `.html` version.
- `stereo_reconstruction/stereo_charuco_calibration_production.py` - Stereo ChArUco detection. Board creation delegated to `calibration_io.create_charuco_detector()`. `ARUCO_DICT_MAP` re-exported for backward compat. **Y-frame convention (2026-04-17):** `make_object_points` (line 301-303) negates the Y column of `board.getChessboardCorners()` before the matched points flow into `cv2.solvePnP`/`stereoCalibrate` so the stored `(R, t)` is physics-up. OpenCV's `cv2.aruco.CharucoBoard` frame has +Y image-down by construction (corner (0,0) at top-left of printed pattern); the negation restores lab-frame convention at calibration construction time.
- `calibration/calibration_stepped/stepped_calibration_production.py` - Stepped board stereo calibration (2-camera). Detects dots on two Z-levels (peak/trough) per camera, assigns absolute grid indices via user fiducials (origin + X/Y axis clicks), builds 3D-2D correspondences with per-level Z values from board geometry. **Always runs multi-view (multi-pose sequence)**. **Operator-declared per-pose peak/trough labels**: `_fit_per_camera_pinhole` requires a `pose_level_labels_by_position: list[str]` parameter — one `'peak'`/`'trough'` entry per pose. The datum pose's entry is cross-checked against the fiducial-derived datum A_label; mismatch is a hard error. **GUI click-to-label (2026-04-16)**: Non-datum poses are verified via click-to-label — user clicks a dot they know is on the peak (or trough) face, backend `identify_pose_level` endpoint snaps to nearest blob and reports which level (A/B) it belongs to, frontend maps to peak/trough using the datum convention. Labels persist to `config.yaml` as `cam*_pose_levels` entries — a pose with no entry is unverified. The "Generate Model" button is disabled until all poses are verified. **`fit_pinhole` multi-image Zhang init (2026-04-16)**: the standalone `fit_pinhole()` function uses a two-step approach: (1) extract one coplanar z-level from ALL poses, feed each pose's subset as a separate "image" to `cv2.calibrateCamera(K=None)` — OpenCV solves the overdetermined IAC system from all homographies jointly, the same mechanism that makes dotboard/charuco robust at PIV magnification; (2) refine with all views using true 3D coordinates (both z-levels) via `CALIB_USE_INTRINSIC_GUESS`. Previous version used only 1 level from 1 pose for Zhang init, which was fatally fragile at PIV magnification (fx=3956 instead of 68321 for transmission cam2). **`_fit_per_camera_pinhole` helper** (method on `SteppedCalibrator`) runs the per-pose accumulation loop and calls `fit_pinhole` for one camera at a time, shared with `SteppedPlanarCalibrator` — zero duplication. **`stitch_levels_pose_local` orientation fix (2026-04-16)**: Fixed a bug where the reference level's grid indices were used raw (un-oriented) while the other level's were orientation-corrected, causing coordinate frame mismatch in the anchor offset computation. Both levels are now oriented identically via `_orient()` before cross-level anchoring. Before fix: 7% consensus and 638px RMS on non-datum poses when `row_sign=-1`. After fix: 93% consensus and 1.31px RMS. **Cannot use `cv2.stereoCalibrate`** because cam1 and cam2 may see different faces of the board at different Z-planes (transmission setup) — no common 3D points exist across cameras. Instead derives stereo pose from individual `cv2.solvePnP` per camera: `R_stereo = R2 @ R1.T`, `T_stereo = t2 - R_stereo @ t1`. Supports `same_side` and `transmission` configurations via `compute_z_and_offsets()`. The same_side/transmission auto-detect (`stereo_config='auto'` fits cam2 twice and picks the lower-RMS config) is active. `cv2.stereoRectify` IS called (only needs R, T, K, dist — not common points) to provide P1/P2/R1/R2 fields required by stereo reconstruction. Clears stale `self_calibration.yaml` on new model generation. Saves per-camera pinhole models + stereo model .mat. **No camera_placement.png** — only `.html` (interactive Plotly) is generated. **Detection overlay (2026-04-16)**: Backend includes datum frame detection data in sequence detection job completion; per-pose detection fetched on demand via `GET sequence_pose_detection`. Overlay colors are pose-level-aware: blue = peak, red = trough, with real-time color swap when a label is set.
- `calibration/calibration_stepped/stepped_planar_calibrator.py` - Stepped board single-camera 3D calibration. `SteppedPlanarCalibrator` subclasses `SteppedCalibrator` and emits per-camera pinhole models to `{base}/calibration/Cam{N}/stepped_planar/model/camera_model.mat` by redirecting via `_subdir_name = "stepped_planar"`. **Uses BOTH Z levels** per pose (not one as the old planar path did) — each pose's dots at two Z planes are genuine non-coplanar 3D points for `cv2.calibrateCamera`, giving real depth information and breaking the fx↔tz ridge without needing a stereo pair. Same operator input as stereo: `generate_camera_model` takes `clicked_level` + `frame_indices` + `pose_levels: dict[int, str]` (per-pose peak/trough labels for THIS camera). Calls the inherited `_fit_per_camera_pinhole` helper with those labels, one camera per call. Multi-camera dispatch goes through the views-layer `ThreadPoolExecutor` (one calibrator instance per camera, parallel). No stereo composition is performed.
- **Config.yaml schema for stepped calibration** — the following keys under `calibration.stepped_board` (stereo) and `calibration.stepped_planar` (per-camera) are populated by the GUI and consumed by both `/generate_model_sequence` Flask routes and the `detect-stepped-stereo` / `detect-stepped-planar` CLI commands:
  - `stepped_board.cam1_fiducials` / `cam2_fiducials`: `{origin, x_axis, y_axis}` of the snapped fiducial pixel clicks on the datum pose.
  - `stepped_board.cam1_clicked_level` / `cam2_clicked_level`: `'peak'` or `'trough'` — the face the operator's origin click landed on.
  - `stepped_board.cam1_pose_levels` / `cam2_pose_levels`: `{frame_idx: 'peak'|'trough'}` — one entry per verified pose. A pose with no entry is unverified. Required by the backend for model generation; the GUI disables "Generate Model" until all poses have entries. Labels are set via click-to-label in the GUI and persist automatically. The CLI reads them directly and fails if any frame in the sequence is missing.
  - `stepped_planar.clicked_level`: `{cam_num: 'peak'|'trough'}`.
  - `stepped_planar.pose_levels`: `{cam_num: {frame_idx: 'peak'|'trough'}}` — same semantics as stereo.
  - `stepped_planar.fiducials`: `{cam_num: {origin, x_axis, y_axis}}`.
  - Board geometry (`dot_spacing_mm`, `step_height_mm`, `board_thickness_mm`, `dt`, `camera_pair`, `datum_camera`, `datum_frame`, `num_calibration_frames`) — `sequence_start_frame` removed (2026-04-16), detection always starts at frame 1.
  - **Additional Flask routes (2026-04-16)**: `GET /sequence_pose_detection` (fetch detection overlay for any pose from sequence cache), `POST /identify_pose_level` (snap click to nearest blob, return which level A/B it belongs to). Available on both `stepped_board_bp` and `stepped_planar_bp`.
- `stereo_reconstruction/stereo_reconstruction_production.py` - Willert/Soloff geometric 3D velocity reconstruction from stereo pairs. **Why not triangulation:** The original approach used `cv2.triangulatePoints` which requires corresponding points (same physical location seen by both cameras). PIV grids are in each camera's own pixel space — grid point (i, j) in cam1 and cam2 are NOT the same physical location because the cameras view the scene from different angles. Pairing by array index produced 307mm Z variation on a flat sheet and 6:1 coordinate aspect ratio. **Coordinates:** Single-camera ray-plane intersection — cam1's PIV grid (uncalibrated pixels → raw pixels → `_pixels_to_world_mm` with Z=z_world + tilt plane). This is the same projection function used by planar pinhole calibration, ensuring coordinate consistency between planar and stereo outputs. No stereo correspondence needed for coordinates — one camera defines the world grid. **Cross-camera correspondence:** Cam1's world grid points are projected into cam2's image via `cv2.projectPoints`, then cam2's displacement field is interpolated at those projected locations using `RegularGridInterpolator`. This establishes correct physical matching — the same world point is queried in both cameras. **Velocities:** The 2×3 projection Jacobian `d(pixel)/d(world)` is computed numerically for each camera at each grid point (perturb ±delta in X, Y, Z → 6 `cv2.projectPoints` calls per camera, central differences). Stacking both cameras gives a 4×3 overdetermined system: `[J1; J2] @ [Ux, Uy, Uz] = [dx1, dy1, dx2, dy2]`, solved via least-squares (normal equations, batched). Uz negated on output (board-Z toward cameras → physical-Z away). Self-calibration params (z_world, tilt_x, tilt_y) threaded through coordinate projection and Jacobian evaluation. Works for all stereo configurations (same-side, transmission, asymmetric). Uses ProcessPoolExecutor for parallel frame processing. Key functions: `_project_coords_to_world()`, `_interpolate_cam2_displacements()`, `_compute_projection_jacobian()`, `_reconstruct_3d_velocities()`.
...
```
GET    /list_videos             ?base_path= -> {videos: str[], videos_meta: [{path, filename, size_mb, modified, data_source}]}
POST   /config                  {camera, data_source, variable, run, cmap, lower, upper, fps, crf, resolution}
POST   /start_video             {base_path_idx, test_mode?, test_frames?}
GET    /video_status            -> {processing, progress, status, out_path?, computed_limits?}
GET    /download                ?path= -> video file stream with range support
DELETE /delete_video            {path} -> deletes video file (validates within allowed roots)
GET    /check_data_sources      ?base_path=&camera= -> availability of calibrated/uncalibrated/merged/stereo/inst_stats
GET    /available_variables     ?base_path=&camera=&data_source= -> {instantaneous: str[], instantaneous_stats: str[]}
GET    /check_runs              ?base_path=&camera=&data_source=&var= -> {runs: int[], highest_run: int}
GET    /constraints             -> {allowed_source_endpoints, ensemble_blocked}
POST   /cancel_video            -> cancels running video job
```

**`video_maker.py`:**
```python
class VideoMaker:
    __init__(base_dir, camera, config)
    process_video(variable, run, data_source, fps, crf, resolution, cmap, lower_limit, upper_limit, ...) -> dict
make_video_from_scalar(folder, var, pattern, settings, cancel_event, run_index) -> dict
find_all_valid_runs_from_file(file_path, var_name="piv_result") -> List[int]
```

**Stereo data handling (2026-04-16):** `_select_variable_from_arrs()` uses dynamic `mask_idx = C - 1` to find b_mask (always last channel). For C=4 stereo data `[ux, uy, uz, b_mask]`, uz is at index 2 and b_mask at index 3. Supports `uz` as a named variable. `mag` includes uz for stereo: `sqrt(ux^2+uy^2+uz^2)`. NaN values merged into mask (prevents NaN->blue artifact).

**Resolution options:** Native (no upscale), 1080p (default), 4K. Minimum dimension guard auto-upscales tiny grids (H<480) to prevent yuv420p chroma subsampling artifacts.

**Quality presets:** Standard (CRF 18), High (CRF 15, default), Publication (CRF 8, near-lossless).

**Descriptive filenames:** `run{N}_{cam}_{var}_{cmap}_{limits}_{source}{_test}.mp4` — e.g., `run1_Cam1_Cam2_uz_jet_auto_stereo.mp4`
...
|----------|-------------|
| `get_filter_specs(config)` | Returns the full ordered list of filter specs from config (preserves user-defined interleaved ordering) |
| `create_filter_pipeline(images, config, pixel_mask, ...)` | `map_blocks(apply_all_filters_slim)` — lazy filtering |
| `apply_all_filters_slim(block, filter_specs, pixel_mask, ...)` | Applies pixel mask then all filters in user-defined order (spatial and temporal interleaved) |
...
**Filter types (applied in user-defined order):**
- **Spatial:** gaussian (sigma), median (size), norm, maxnorm, lmax, invert, clahe (all via `scipy.ndimage`)
- **Temporal:** pod (SVD-based), time
- Filters are applied in the exact order specified by the user in the GUI/CLI stack. Use `get_filter_specs()` to retrieve the ordered list.
...
| `import-davis-pinhole <xml_path>` | Import DaVis PinholeOpenCV Calibration.xml → saves `davis_pinhole/model/camera_model.mat` per camera, sets `calibration.active = davis_pinhole`. Options: `--dt`, `--base-path-idx`, `--camera-map 1:1,2:2,3:3`. Auto-discovers `camera1/MarkPositionTable.xml` in the same directory; if found, uses `cv2.solvePnP` on the ~590 pixel↔world mark correspondences to fit accurate rvec/tvec (corrects DaVis 1-view degeneracy and global-optimisation residuals for all cameras). |
...
See `profile/README.md` for the full script catalogue, the benchmark suite (`run_all_benchmarks.py`), and usage details. Benchmark outputs (CSVs + PNGs) are written to `profile/results/` (gitignored). The section below covers the fine-grained profilers only.
...
- `paper_submission/` — Main benchmark scripts (planar + stereo)

### `paper_submission/benchmark_comparison.py` — Planar & Ensemble Benchmark
...
python -m paper_submission.benchmark_comparison \
    --mode instantaneous \
...
```python
from paper_submission.benchmark_comparison import (
...
### `paper_submission/stereo_benchmark_comparison.py` — Stereo 3-Component Benchmark
...
- **Calibration source change resets derived fields:** When `calibration_sources` changes in `update_config`, ALL derived calibration fields are auto-reset: global coordinate pixel-dependent fields (`datum_pixel`, `overlap_points`, `overlap_pairs`, `invert_ux`, `datum_camera`), polynomial coefficients (`polynomial.cameras → {}`), stepped board/planar fiducials + clicked levels + pose levels, and self-calibration config copy. User-intent fields (board geometry, `enabled`, `datum_physical`, `datum_frame`) are preserved. Guard: no reset on first-time setup (empty → populated).
- **Active method change clears derived data:** When `calibration.active` changes, `self_calibration` config copy is cleared. Switching AWAY from polynomial also clears `polynomial.cameras` (prevents stale coefficients crowding the YAML).
- **Self-calibration stored in file only:** Results are saved to `{base_path}/calibration/stereo_cam{A}_cam{B}/self_calibration.yaml` (canonical). `config.py:self_calibration_config` reads from this file. No backward-compat copy in config.yaml.
...
- **`vmin_pct`/`vmax_pct` convention:** Always `0.0`/`100.0` — images are contrast-optimised by `numpy_to_base64` (sqrt + percentile clip) before encoding. The frontend slider works on the 8-bit range directly.
...
- **Coordinate convention invariant (planar + stereo + polynomial):** OpenCV calibration models are fitted with raw pixel coordinates (0-based, y-down). All OpenCV functions (`_pixels_to_world_mm`, `cv2.undistortPoints`, `cv2.projectPoints`) **must** receive raw pixels. PIVTOOLs stores uncalibrated coords (1-based, y-up). Conversion: `x_raw = x_uncal - 1`, `y_raw = image_height - y_uncal`. The `image_height` is loaded from the model .mat file's `image_size` field. **Y-axis physics-up convention — where the negation happens depends on the path:** *(a) Planar pinhole + polynomial + planar ChArUco* negate at output time: `vector_calibration_production.py` (`y_mm = y_sign*world[:,1]`, `uy_ms = y_sign*delta[:,1]` where `y_sign=-1` by default) and `polynomial_calibration_production.py` (`y_mm = -y_world_px * mm_per_pixel`, `v_ms = -v_world_mm`). *(b) Regular stereo dotboard* negates at calibration construction time: `stereo_dotboard_calibration_production.py:215,271` negate `obj_points[:, 1]` before `cv2.solvePnP` / `cv2.stereoCalibrate` so the stored `(R, t)` is already physics-up (fixed 2026-04-17). *(c) Stereo ChArUco* likewise negates the Y column of `board.getChessboardCorners()` in `stereo_charuco_calibration_production.py:301-303` (OpenCV's ChArUco board frame is +Y image-down by construction). *(d) Stepped stereo* uses fiducial-defined world frame (user +X/+Y clicks on datum pose); already physics-up by construction. *(e) `stereo_reconstruction_production.py`* is frame-agnostic — it forwards whatever world frame the stored model specifies, so no negation there. *(f) **DaVis pinhole import** (`davis_pinhole`)* — DaVis stores `RotationAngles Rx≈±π`, which is a ~180° rotation around X that **already encodes a y-up world frame** in the stored `(R, t)`. The import saves `y_negate: 0` in the model `.mat` file; `VectorCalibrator` reads this flag and sets `y_sign=+1` (no negation). All other native model types default to `y_negate: 1` (`y_sign=-1`). **Gotcha:** re-importing a DaVis XML generates a new model with the correct flag; old models imported before this fix have no `y_negate` field and default to `y_sign=-1` (producing the wrong sign) — delete and re-import them. **1-view degeneracy + mark-based fitting:** DaVis panoramic (SideBySide2D) calibrations with one plate pose are degenerate — f and Tz are individually unobservable; only f/Tz is correct. Camera 1 can converge to near-zero f_px (e.g. 2.56px), causing a denominator sign-flip mid-sensor that sends world coords to ±∞. Fix: `_maybe_normalise_focal()` detects cameras with unsafe ray-plane denominators at image corners and sets f_px=W/2 (Tz scaled proportionally). More importantly, if `camera1/MarkPositionTable.xml` exists next to Calibration.xml, `_fit_with_solvepnp()` runs `cv2.solvePnP` on ~590 pixel↔world mark correspondences for each camera, correcting both the degenerate camera 1 and residual (~7mm) DaVis optimisation errors in other cameras. Always re-import after moving or renaming these files. **Stereo reconstruction (Willert/Soloff):** Does NOT use `cv2.triangulatePoints` — triangulation requires corresponding points across cameras, but PIV grid indices don't correspond (same index = different physical location because cameras have different viewpoints). Instead: coordinates from single-camera ray-plane intersection (`_project_coords_to_world` → `_pixels_to_world_mm`), same function as planar calibration. Cross-camera correspondence established by projecting cam1's world grid into cam2's image (`cv2.projectPoints`) and interpolating cam2's displacements there (`_interpolate_cam2_displacements`). Velocities from 4×3 Jacobian least-squares (`_reconstruct_3d_velocities`). Only Uz is negated in the reconstruction module (board-Z toward cameras → physical-Z away); Y handling is done upstream by whichever stereo calibrator produced the model.
- **Stepped board stereo pose derivation:** `cv2.stereoCalibrate` requires common 3D points visible to both cameras. The stepped board in transmission mode has no common points (each camera sees a different face at a different Z). Stereo pose is derived from individual `cv2.solvePnP` results: `R_stereo = R2 @ R1.T`, `T_stereo = t2 - R_stereo @ t1`. `cv2.stereoRectify` is still called (needs only R, T, K, dist) to provide rectification fields for reconstruction. Both cameras must use consistent world coordinates — the fiducial-based grid index assignment (`assign_absolute_grid_indices`) ensures this by aligning each camera's BFS grid to the user's physical axis clicks.
